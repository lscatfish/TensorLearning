...
# GridSearch: alpha vs hidden_layer_sizes 折线对比
...
colors_arch = plt.cm.viridis(np.linspace(0.2, 0.8, len(param_grid["hidden_layer_sizes"])))
for i, arch in enumerate(param_grid["hidden_layer_sizes"]):
    ax.plot(range(len(param_grid["alpha"])), score_matrix[i], "o-",
...
# MLP 不同架构训练曲线（独立图，属于 MLP 内部）

print("  [11/11] MLP 训练曲线对比...")
